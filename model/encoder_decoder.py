...
#checkpoint恢复点
check_points=120000
checkpoint_iter =check_points+1

#语料集、分词路径
file_path = os.path.abspath('../resources/chat_corpus')
seq_path = os.path.abspath('../resources/segments')
corpus_name = "whale_dream"
chat_corpus_name='鲸梦.csv'
corpus = os.path.join(file_path, chat_corpus_name)
...
# 配置模型
model_name = '{}_chatbot_model'.format(corpus_name)
attn_model = 'dot'
#attn_model = 'general'
#attn_model = 'concat'
hidden_size = 500
...
#loadFilename =None#如果是None，那么从头开始训练。
loadFilename = os.path.abspath('./data/{}/{}/2-2_500/{}_checkpoint.tar'.format(model_name,corpus_name,check_points))
...
printLines(os.path.join(file_path, chat_corpus_name))
word_wise = False  # True: 分词， False：分字
word_segmentation_path = os.path.join(seq_path, '{}_seg.txt'.format(corpus_name))  # 保存分词后的结果
...
cut_sentences(os.path.join(file_path, chat_corpus_name), word_segmentation_path)
...
def trainIters(model_name, voc, pairs, encoder, decoder, encoder_optimizer, decoder_optimizer,
    ...
    for iteration in range(start_iteration, n_iteration + 1):
        ...
        if iteration % print_every == 0:
            ...
            writer.add_scalar('Average_loss', round(print_loss_avg,4),iteration)
